...
텍스트를 입력하면 360도 HDRI 와 PBR 채널을 생성하고 검증한다.

## 설치

1. 외부 모델 clone 과 패치 (리포 폴더 안에서 실행)
...
2. conda 환경 dit360 (생성, HDR, OmniX, 파생 채널)
...
./run.sh inputs/text/01_sunny_plaza.txt  # 입력 하나만 처리
python scripts/validate.py outputs       # 검증만 다시 실행
```

입력은 inputs/text 폴더의 txt 파일 한 줄 텍스트이다.
...
- pbr/final/: basecolor, normal, roughness, metallic, ao, height,
  displacement, depth, material_id, semantic 의 png
