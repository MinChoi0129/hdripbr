...
def expand_to_hdr(pred, ldr, kmax=300.0, thr_lo=0.83, thr_hi=0.99, sky_k=6.0):
    """ExpandNet 출력([0,1], sigmoid로 제한됨)을 진짜 HDR(값>1)로 확장.

    네트워크는 클리핑 영역의 '디테일'은 복원하나 절대 동적범위는 [0,1]에 갇힌다.
    입력 LDR의 포화(클리핑)·고휘도 영역을 광원 후보로 보고 지수적으로 1.0 위로 확장한다.
    pred, ldr 모두 BGR float [0,1].
    """
    import numpy as np
    def _smoothstep(lo, hi, x):
        ...
        return t * t * (3 - 2 * t)
    # BGR 휘도
    lum_ldr = 0.0722 * ldr[..., 0] + 0.7152 * ldr[..., 1] + 0.2126 * ldr[..., 2]
    lum_pred = 0.0722 * pred[..., 0] + 0.7152 * pred[..., 1] + 0.2126 * pred[..., 2]
    lum = np.maximum(lum_ldr, lum_pred)
    hi_mask = _smoothstep(thr_lo, thr_hi, lum) ** 2.0          # 광원/포화
    mid_mask = _smoothstep(0.5, thr_lo, lum)                   # 하늘/밝은면
    boost = 1.0 + hi_mask * (kmax - 1.0) + mid_mask * (sky_k - 1.0)
    return (pred * boost[..., None]).astype('float32')
